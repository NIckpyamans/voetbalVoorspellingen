...
const MatchCard: React.FC<MatchCardProps> = ({ match, prediction, onFavoriteChange }) => {
  ...
  const renderAdvancedStats = () => {
    ...
    return (
      <div className="space-y-3 mt-2 p-4 bg-slate-800/60 rounded-lg text-xs border border-slate-600/50 text-slate-200">
        {/* Injuries */}
        {(match.homeInjuries || match.awayInjuries) && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 mb-1.5 font-semibold text-[11px]">Blessures & Schorsingen</div>
        ...
        {match.h2h && match.h2h.played >= 2 && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 mb-1.5 font-semibold text-[11px]">Head-to-Head ({match.h2h.played}x)</div>
            <div className="flex justify-between mb-2">
              <span className="text-green-300">{match.h2h.homeWins}W</span>
              <span className="text-yellow-300">{match.h2h.draws}D</span>
              <span className="text-red-300">{match.h2h.awayWins}W</span>
            </div>
            {match.h2h.results && match.h2h.results.length > 0 && (
              <div className="mt-2 space-y-1 bg-slate-900/30 rounded p-2">
                <div className="text-[10px] text-slate-400 font-semibold mb-1">📊 Laatste 5 duels:</div>
                {match.h2h.results.slice(-5).reverse().map((result: any, i: number) => (
                  <div key={i} className="flex justify-between items-center text-[10px] py-0.5">
                    <span className="text-slate-400 font-mono">{result.date?.split('-').slice(0, 2).reverse().join('/')}</span>
                    <span className="text-slate-200 font-semibold">{result.score}</span>
        ...
        {match.weather && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 mb-1.5 font-semibold text-[11px]">🌤️ Weer</div>
            <div className="flex justify-between text-slate-200">
              {match.weather.temperature && <span>{match.weather.temperature}°C</span>}
              {match.weather.windSpeed && <span>💨 {match.weather.windSpeed} km/u</span>}
              {match.weather.precipitationProbability && (
                <span>☔ {match.weather.precipitationProbability}%</span>
              )}
            </div>
          </div>
        )}
        ...
        {(match.homeTeamProfile?.setPieceScore || match.awayTeamProfile?.setPieceScore) && (
          <div className="flex justify-between pb-2 border-b border-slate-700/50">
            <span className="text-slate-200">{match.homeTeamProfile?.setPieceScore ?? "-"}</span>
            <span className="text-slate-400 font-semibold">⚽ Set Pieces</span>
            <span className="text-slate-200">{match.awayTeamProfile?.setPieceScore ?? "-"}</span>
          </div>
        )}
        ...
        {match.referee?.name && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 font-semibold text-[11px]">👨‍⚖️ Scheidsrechter</div>
            <div className="text-slate-200">{match.referee.name}</div>
        ...
        {prediction?.modelEdges && (
          <div className="space-y-1.5 pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 font-semibold text-[11px]">🤖 Model Insights</div>
            {prediction.modelEdges.riskProfile && (
              <div className="text-slate-200">
                Risico: <span className={`font-bold ${
                  prediction.modelEdges.riskProfile === "high" ? "text-red-300" :
                  prediction.modelEdges.riskProfile === "medium" ? "text-yellow-300" :
                  "text-green-300"
            ...
            {prediction.modelEdges.modelAgreement != null && (
              <div className="text-slate-200">
                Model agreement: <span className="font-bold text-blue-300">{Math.round(prediction.modelEdges.modelAgreement * 100)}%</span>
              </div>
            )}
            {prediction.modelEdges.tacticalMismatch?.summary && (
              <div className="text-[10px] text-blue-200 bg-blue-900/20 rounded px-2 py-1">
        ...
        {match.lineupSummary?.confirmed && (
          <div className="text-green-300 font-semibold pb-2 border-b border-slate-700/50">
        ...
        {match.venue?.name && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 font-semibold text-[11px]">🏟️ Stadion</div>
            <div className="text-slate-200">{match.venue.name}</div>
        ...
        {match.aggregate?.active && (
          <div className="pb-2 border-b border-slate-700/50">
            <div className="text-slate-400 font-semibold text-[11px]">🔄 Tweeluik</div>
            <div className="text-slate-200">
        ...
        {match.context?.summary && (
          <div>
            <div className="text-slate-400 font-semibold text-[11px]">📋 Context</div>
            <div className="text-yellow-200 bg-yellow-900/20 rounded px-2 py-1 mt-1">{match.context.summary}</div>
          </div>
        )}
      </div>
    );
