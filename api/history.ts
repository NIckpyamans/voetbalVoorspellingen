...
async function readDatabaseHistoryFallback(limit = 1500) {
  ...
  return rows.map((row: any) => {
    ...
    const predHome = row.payload_pred_home_goals ?? expected.home ?? null;
    const predAway = row.payload_pred_away_goals ?? expected.away ?? null;
    return {
      matchId: row.match_id,
      predictionId: row.prediction_id,
      predictedScore: predHome != null && predAway != null ? `${predHome}-${predAway}` : expected.label || null,
      actualScore: row.payload_actual_score || null,
      ...
      homeTeamName: row.payload_home_team || row.input_home_team || null,
      awayTeamName: row.payload_away_team || row.input_away_team || null,
      league: row.payload_league || row.input_league || null,
      predictedOutcome: row.payload_predicted_outcome || null,
      actualOutcome: row.payload_actual_outcome || null,
