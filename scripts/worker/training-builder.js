export function buildTrainingSnapshot(store, { isHiddenEntity, selectPredictionForReview } = {}) {
  ...
  for (const date of Object.keys(store.matches || {})) {
    ...
    for (const match of matches) {
      ...
      const snapshotCandidates = (snapshotsByMatchId.get(match.id) || []).filter((snapshot) => snapshot?.predictionId && snapshot?.generatedAt).map((snapshot) => ({
  ...
  for (const snapshots of Object.values(store.predictionSnapshots || {})) {
    for (const snapshot of Array.isArray(snapshots) ? snapshots : [snapshots]) {
      if (!snapshot?.predictionId || !snapshot?.matchId || emittedPredictionIds.has(snapshot.predictionId) || isHiddenEntity?.(snapshot)) continue;
  ...
  return { generatedAt: new Date().toISOString(), version: "v9-snapshot-expanded", reviewCount: Object.keys(store.postMatchReviews || {}).length, rows };
}
