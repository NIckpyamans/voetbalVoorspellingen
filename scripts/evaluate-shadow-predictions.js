...
const profiles = await sql.query("select * from calibration_profiles where phase_bucket='competition_recalibration_candidate'");
let evaluated = 0;
for (const profile of profiles) {
  ...
  for (const row of rows) {
    const raw = ["home", "draw", "away"].map((key) => Math.max(0, Number(row.probabilities?.[key] || 0)));
    const total = raw.reduce((sum, value) => sum + value, 0);
    if (!total) continue;
    const current = raw.map((value) => clamp(value / total));
    const shadow = current.map((value, index) => clamp(value * (1 - shrink) + Number(prior[["home", "draw", "away"][index]] || 0) * shrink));
    const shadowTotal = shadow.reduce((sum, value) => sum + value, 0);
    const normalizedShadow = shadow.map((value) => value / shadowTotal);
